...
plot_SA_Figures_v2 = function(IVM_traj_Control,IVM_traj_cc,IVM_traj_LLIN_50,IVM_traj_Cattle_50,IVM_traj_LLIN_HOUSE_50,IVM_traj_LLIN_OBT_50,theta,INITIAL_MODELRUNTIME_VALUE){
  #. plot_SA_Figures - produce some figures for presentations
  impactFeedingCycle = impactFeedingCycleParameters(INITIAL_MODELRUNTIME_VALUE, theta[["beta"]], theta[["tau1"]],theta[["tau2"]],theta[["e_ov"]],theta[["time_ATSB_on"]],theta[["ATSBcov"]],theta[["time_SSP_on"]],
                                                    theta[["SSPcov"]],theta[["fSSP"]],theta[["fATSB"]],theta[["muV"]],theta[["Q0"]],theta[["aOBT"]],theta[["OBTcov"]],theta[["time_OBT_on"]],theta[["time_ITN_on"]],
                                                    theta[["ITNcov"]],theta[["time_IRS_on"]],theta[["IRScov"]],theta[["HOUcov"]],theta[["time_HOU_on"]],theta[["rITN"]],theta[["sITN"]],theta[["rIRS"]],theta[["rHOU"]],
  ...
  NV_cc = IVM_traj_cc[["SV"]]+IVM_traj_cc[["EV"]]+IVM_traj_cc[["IV"]]
  
  NV_LLIN_50 = IVM_traj_LLIN_50[["SV"]]+IVM_traj_LLIN_50[["EV"]]+IVM_traj_LLIN_50[["IV"]]
  NV_cattle_50 = IVM_traj_Cattle_50[["SV"]]+IVM_traj_Cattle_50[["EV"]]+IVM_traj_Cattle_50[["IV"]]
  
  NV_LLIN_HOUSE_50 = IVM_traj_LLIN_HOUSE_50[["SV"]]+IVM_traj_LLIN_HOUSE_50[["EV"]]+IVM_traj_LLIN_HOUSE_50[["IV"]]
  
  NV_LLIN_OBT_50 = IVM_traj_LLIN_OBT_50[["SV"]]+IVM_traj_LLIN_OBT_50[["EV"]]+IVM_traj_LLIN_OBT_50[["IV"]]
  ...
  p <- ggplot(IVM_traj_Control, aes(x = time, y = IVM_traj_Control, color = Key)) +
    geom_line(aes(y = NV_Control, col = "Mosquito-Control"), size = 1.2) + 
    geom_line(aes(y = NV_LLIN_50, col = "Mosquito-LLIN-50"), size = 1.2) +
    geom_line(aes(y = NV_cattle_50, col = "Mosquito-Cattle-50"), size = 1.2) +
    geom_line(aes(y = NV_cc, col = "Mosquito-LLIN 50 & Cattle 50"), size = 1.2) +
    geom_line(aes(y = NV_LLIN_HOUSE_50, col = "Mosquito-LLIN-50-ModifiedHouse50"), size = 1.2) +
    geom_line(aes(y = NV_LLIN_OBT_50, col = "Mosquito-LLIN-BaitedTrap-50"), size = 1.2) +
    labs(title = "An. Arabiensis", x = "Time (days)", y = "Number of mosquitoes")
  p <- p + expand_limits(x = 0, y = 0)
  multiplot(p, cols=1)
  
  #Bar plots??
  NumMosq <- c(sum(NV_Control),sum(NV_cc),sum(NV_LLIN_50),sum(NV_LLIN_HOUSE_50),sum(NV_cattle_50),sum(NV_LLIN_OBT_50))
  ...
  #IV
  IV_Control = IVM_traj_Control[["IV"]]
  IV_cc = IVM_traj_cc[["IV"]]
  IV_LLIN_50 = IVM_traj_LLIN_50[["IV"]]
  IV_Cattle_50 = IVM_traj_Cattle_50[["IV"]]
  IV_LLIN_HOUSE_50 = IVM_traj_LLIN_HOUSE_50[["IV"]]
  IV_LLIN_OBT_50 = IVM_traj_LLIN_OBT_50[["IV"]]
  ...
  time_on = 20                 #Manually set it to the earliest time an intervention is on - to be automated
  EIR_control <-computeEIR(time_on,a_theta, IV_Control, NH)
  EIR_CC <-computeEIR(time_on,a_theta, IV_cc, NH)
  EIR_LLIN_50 <-computeEIR(time_on,a_theta, IV_LLIN_50, NH)
  EIR_Cattle_50 <-computeEIR(time_on,a_theta, IV_Cattle_50, NH)
  EIR_LLIN_HOUSE_50 <-computeEIR(time_on,a_theta, IV_LLIN_HOUSE_50, NH)
  EIR_LLIN_OBT_50 <-computeEIR(time_on,a_theta, IV_LLIN_OBT_50, NH)
  ...
  p2 <- ggplot(IVM_traj_Control, aes(x = 1:length(IVM_traj_Control[,1]), y = EIR, color = Key)) +
    geom_line(aes(y = EIR_control , col = "EIR - Control "), size = 1.2) +
    geom_line(aes(y = EIR_LLIN_50, col = "EIR - LLIN - 50"), size = 1.2) + 
    geom_line(aes(y = EIR_Cattle_50, col = "EIR - Cattle - 50"), size = 1.2) + 
    geom_line(aes(y = EIR_CC, col = "EIR - LLIN 50 & Cattle 50 "), size = 1.2) + 
    geom_line(aes(y = EIR_LLIN_HOUSE_50, col = "EIR - LLINs-ModifiedHouse - 50"), size = 1.2) +
    geom_line(aes(y = EIR_LLIN_OBT_50, col = "EIR - LLINs-BaitedTraps- 50"), size = 1.2) +
    labs(title = "An. Arabiensis", x = "Time (days)", y = "EIR")
  ...
  browser()
  #dont sum EIR
  #EIR_ALL <- c(sum(EIR_control),sum(EIR_CC),sum(EIR_LLIN_50),sum(EIR_Cattle_50),sum(EIR_LLIN_HOUSE_50),sum(EIR_LLIN_OBT_50))
  EIR_ALL <- c(EIR_control[length(EIR_CC)],EIR_CC[length(EIR_CC)],EIR_LLIN_50[length(EIR_CC)],EIR_Cattle_50[length(EIR_CC)],EIR_LLIN_HOUSE_50[length(EIR_CC)],EIR_LLIN_OBT_50[length(EIR_CC)])
  barplot(EIR_ALL , main='An. Arabiensis EIR = 100', xlab='Tools',ylab='EIR',names.arg=c('AA','BB','CC','DD','EE','FF'))
...
#plot the trajectory with ggplot2 
