...
plot_MS_Figures = function(IVM_traj_Control,IVM_traj_LLIN_60,IVM_traj_LLIN_30,theta,INITIAL_MODELRUNTIME_VALUE){
  ...
  # 
   p2 <- ggplot(IVM_traj_Control, aes(x = 1:length(IVM_traj_Control[,1]), y = EIR, color = key)) +
     ...
     geom_line(aes(y = IVM_traj_LLIN_50[["EIR"]], col = "EIR - ITNs - 0.50 "), size = 1.2) + 
     geom_line(aes(y = IVM_traj_LLIN_80[["EIR"]], col = "EIR - ITNs - 0.80"), size = 1.2) + 
