...
runODE = function(tMax, tIncrement, initState, theta, method){
  ...
  IVM_traj  <- data.frame(dede(y = initState, times = times, parms = theta, func = IVM_ode, method = method))
  return(IVM_traj)
}
######################################################################################
calculateInitialState = function(theta){
  ...
  epsilon0  <- theta[["epsilon0"]]; recRate   <- theta[["recRate"]];  NH_eq     <- theta[["NH_eq"]];
  bV        <- theta[["bV"]];       bh        <- theta[["bh"]];
...
IVM_ode <- function(time, state, theta){
  ...
  #Introduce the impact of ovitrap on eggs laying
  ...
  if(cLAR == 0){cLAR = 2*exp(-16)}
  if(cBIO == 0){cBIO = 2*exp(-16)}
  if(c_LAR_BIO == 0){c_LAR_BIO = 2*exp(-16)}
  if(c0_LAR_BIO == 0){c0_LAR_BIO = 2*exp(-16)}
  f_LAR_BIO = fLAR*fBIO
  #################**********************************
  ##********************************* ODEs:
  if(time < durEV){
