...
plotTrajectoryPlotLy = function(IVM_traj){
  #. plotTrajectory: Plots the evolution of the dynamics of the system
  #plot=ggplot(IVM_traj, aes(x = time, y = IVM_traj)) +
    #geom_line(aes(y = SV+EV+IV, col = "NV"), size = 1.2) +
    #theme_grey(base_size = 18, base_family = "") +
  #  geom_line(aes(y = IV), size = 1.5) +
  #  geom_line(aes(y = SV), size = 1.5) +
  #  geom_line(aes(y = EV), size = 1.5) +
  #  ylim(0, NA) +
  #  labs(x = "Time (days)", y = "Number of mosquitoes")
  #ggplotly(plot) %>% layout(xaxis = AxisStyle("Time"), yaxis = AxisStyle("Number of Mosquitos"),legend = list(x = 0.5, y = 0))
...
plotEIRPlotLy = function(IVM_traj){
  ...
  # plot=ggplot(IVM_traj, aes(x = time, y = EIR, color = State)) +
  #   theme_grey(base_size = 18, base_family = "") +
  #   geom_line(aes(y = IVM_traj[["EIR"]], col = "EIR"), size = 1.5, colour = 'magenta') +
  #   ylim(0, NA) #+
  #   #labs(x = "Time (days)", y = "Entomological Inoculation Rate (EIR)")
  # ggplotly(plot) %>% layout(xaxis = AxisStyle("Time"), yaxis = AxisStyle("Entomological Inoculation Rate (EIR)"))
...
barChartMosquitoDemographicsPlotLy = function(IVM_traj){
  ...
  #   guides(fill=FALSE) +
  #   labs(x="Mosquito Stage",y="Number of Mosquitoes")
  plot_ly(x = c("Early Instar","Late Instar","Pupae","Susceptible Ad.","Exposed Ad.","Infected Ad."), y = traj_dat$sum,type = "bar", name = list("EL","LL","PL","SV","EV","IV"), color = list("EL","LL","PL","SV","EV","IV")) %>%
